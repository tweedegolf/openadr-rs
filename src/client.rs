...
impl ClientRef {
    pub async fn get<'a, T: serde::de::DeserializeOwned>(&self, path: &str, query: Option<impl serde::Serialize>) -> Result<T> {
    ...
    pub async fn post<'a, S: serde::ser::Serialize, T: serde::de::DeserializeOwned>(&self, path: &str, body: &S) -> Result<T> {
        let url = self.base_url.join(path)?;
    ...
    pub async fn put<'a, S: serde::ser::Serialize, T: serde::de::DeserializeOwned>(&self, path: &str, body: &S) -> Result<T> {
        let url = self.base_url.join(path)?;
...
impl Client {
    ...
    pub async fn create_program(&self, program_data: ProgramContent) -> Result<ProgramClient> {
        ...
        Ok(ProgramClient::from_program(self.client_ref.clone(), program))
    }

    /// Get a list of programs from the VTN with the given query parameters
    pub async fn get_programs(&self, target_type: Option<TargetLabel>, targets: &[&str], skip: usize, limit: usize) -> Result<Vec<ProgramClient>> {
        ...
        Ok(programs.into_iter().map(|program| ProgramClient::from_program(self.client_ref.clone(), program)).collect())
    }
    ...
    pub async fn get_all_programs(&self) -> Result<Vec<ProgramClient>> {
        ...
        loop {
            ...
            let received = self.get_programs(None, &[], page * PAGE_SIZE, PAGE_SIZE).await?;
    ...
    pub async fn get_program_by_name(&self, name: &str) -> Result<ProgramClient> {
        let mut programs = self.get_programs(Some(TargetLabel::ProgramName), &[name], 0, 2).await?;
    ...
    pub async fn get_program_by_id(&self, id: &ProgramId) -> Result<ProgramClient> {
        let program = self.client_ref.get(&format!("programs/{}", id.0), None::<()>).await?;
...
impl ProgramClient {
    ...
    pub async fn update(&mut self) -> Result<()> {
        let res = self.client.put(&format!("programs/{}", self.id()), &self.data.content).await?;
        self.data = res;
        Ok(())
    }
    ...
    pub async fn get_events(&self, target_type: Option<TargetLabel>, targets: &[&str], skip: usize, limit: usize) -> Result<Vec<crate::wire::Event>> {
        // convert query params
        let target_type_str = target_type.map(|t| t.to_string());
        let skip_str = skip.to_string();
        let limit_str = limit.to_string();
        ...
        // send request and return response
        Ok(self.client.get("programs", Some(&query)).await?)
    }
}
